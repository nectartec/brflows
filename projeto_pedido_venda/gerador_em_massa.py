...
# Função para buscar os 5 próximos pedidos
def buscar_cinco_pedidos():
    """
...
def gerar_pdf(nr_pedido, codigo_cli, nome_cli, ender,
              ...
              ValorDesconto, observacao, pedido_id):
    ...
    def desenhar_celula(x, y, largura, altura, texto="", imagem=False, caminho_imagem=None):
        ...
        if texto:
            # Verifica se o texto excede 10 caracteres
            if len(texto) > 14:
                c.setFont("Helvetica", 7)
                # Quebra o texto em múltiplas linhas, cada linha com no máximo 10 caracteres
                linhas = wrap(texto, width=14)
                # Desenha cada linha dentro da célula
                linha_altura = altura / len(linhas)  # Altura para cada linha
                for i, linha in enumerate(linhas):
                    texto_y = y - altura + (linha_altura * (len(linhas) - i - 1)) + 5  # Calcula posição
                    c.drawString(x + 5, texto_y, linha)
            else:
...
# Corpo do e-mail em texto e HTML
    ...
    "<strong>Observação:</strong> Observação<br><br>"  # Texto fixo "Observação"
...
if "email_map" not in st.session_state:
    st.session_state["email_map"] = {}

# Botão para carregar pedidos e exibir a tabela
if st.button("Carregar Pedidos"):
    pedidos = buscar_cinco_pedidos()
    st.session_state["pedidos"] = pedidos
    st.session_state["email_map"] = {pedido[4]: pedido[13] for pedido in pedidos}

# Exibir pedidos se carregados
if st.session_state["pedidos"]:
    st.write("**Pedidos Carregados:**")

    # Transformar pedidos em DataFrame
    colunas = [
        "Subtotal", "Desconto", "Valor Desconto", "Total", "Pedido",
        "Código Cliente", "Nome Cliente", "Endereço", "Cidade", "CEP",
        "CNPJ", "Inscrição Estadual", "Telefone", "Email", "Fax",
        "Transportadora", "Data", "Representante Código", "Nome Representante",
        "Condição Pagamento"
    ]
        ...
        for pedido in st.session_state["pedidos"]:
            try:
                ...
                pdf_buffer = gerar_pdf(
                    ...
                    pedido[16], pedido[17], pedido[18], pedido[3], pedido[0], pedido[1], pedido[2], "",
                    pedido[4]
                )

                # Enviar o e-mail
                resultado = enviar_email(Email, pedido, pdf_buffer)
                st.success(f"Pedido {pedido[4]} enviado com sucesso para {Email}!")
