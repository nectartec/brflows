...
openai.api_key = 'sk-None-z6Vef2nui8yFGUBI8VvlT3BlbkFJGJyGi7vGXXoFBJqWLqFZ'
...
def extrair_informacoes(texto):
    prompt = (
        "Por favor, extraia as seguintes informações da NFe:\n"
        "1. Número da Nota Fiscal (apenas números, sem caracteres especiais ou espaços) - Deve ser o Número da Nota Fiscal, ou ainda, Numero da NFSe, ou número identificado equivalente para identificar a NFSe.\n"
        ...
        "Responda somente com a linha CSV no formato especificado, garantindo que o CNPJ seja o do emitente (prestador) e não do tomador."
    )
...
def verificar_e_extrair(texto):
    ...
    for tentativa in range(max_tentativas):
        ...
        print("Infos extraídas: ", linha_csv)
        
        # Remove caracteres indesejados e quebra de linha
        linha_csv = linha_csv.replace("```", "").replace("\n", "")
        
        campos = [campo.strip() for campo in linha_csv.split(',')]
        
        # Tenta converter o primeiro campo (Número da Nota Fiscal) para int para garantir que seja apenas números
        try:
            campos[0] = str(int(campos[0]))  # Remove zeros à esquerda e caracteres estranhos
        except ValueError:
            print("Erro ao converter o número da NF para inteiro.")
        
        print("Campos após split: ", campos)
        
        if len(campos) == 4:
            return campos
        else:
            print("Entrando em Reprocessamento para a NF")
            aviso_prompt = f"A resposta anterior estava incorreta. Por favor, corrija e formate a informação corretamente como uma linha CSV: Número da Nota Fiscal, Razão Social, CNPJ, Data Emissão.\n{linha_csv}"
            response = openai.ChatCompletion.create(
                model="gpt-4o-mini",
...
def processar_notas_fiscais(diretorio_pdfs, caminho_acompanhamento, output_nf_path, output_massivo_path, reprocessar_path, historico_execucao_path):
    ...
    # Verificar quais NFs estão válidas e quais não estão
    df_notas['valido'] = df_notas.apply(lambda row: any(
        (acompanhamento_df['NF'] == row['Número da Nota Fiscal']) & 
        (acompanhamento_df['CNPJ'] == row['CNPJ'])
    ), axis=1)
